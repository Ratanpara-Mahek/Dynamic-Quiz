...
class AddQuizActivity : AppCompatActivity() {
    ...
    private fun addQuestions() {
        ...
        binding.container.removeViews(6, binding.container.childCount - 6) // Updated to account for new fields
    ...
    private fun addQuiz() {
        ...
        val startTimeStr = binding.startTimeEdit.text.toString().trim()
        val endTimeStr = binding.endTimeEdit.text.toString().trim()
        val numQuestionsStr = binding.numQuestionsEdit.text.toString().trim()

        if (title.isEmpty() || subtitle.isEmpty() || time.isEmpty() || startTimeStr.isEmpty() || endTimeStr.isEmpty() || numQuestionsStr.isEmpty()) {
        ...
        val quizId = FirebaseDatabase.getInstance().reference.push().key ?: return
